...
def do_poll(socket, stream_id, poll_id, poll_item_id, user = None):
    old_poll_item_id = user.do_poll(poll_id, poll_item_id)
    
    # do events
    if(old_poll_item_id!=poll_item_id):
        changes = [PollChangeEvent(poll_id = poll_item_id , count = 1)]
        if(old_poll_item_id!=None):
            changes.append(PollChangeEvent(poll_id = old_poll_item_id , count = -1))
